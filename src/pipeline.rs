...
use crate::text_utils::{
    extract_reference_urls, fingerprint, infer_source_layer, json_ratio, new_run_id, normalize_author,
    normalize_content_type, normalize_metrics, normalize_platform, normalize_source_type,
    normalize_text, normalize_timestamp, normalize_url, semantic_basis, tokenize, utc_now_iso,
};
use serde_json::json;
...
impl SPGInputPipeline {
    ...
    pub fn process(&self, items: Vec<RawSourceItem>) -> PipelineRunResult {
        ...
        for raw in &items {
            ...
            let information_key = format!(
                ...
                event_key,
                fingerprint(
                    &[&semantic_basis(&clean.normalized_title, &clean.normalized_text)],
                    20,
                )
            );
            ...
            if duplicate_type != "none" {
                ...
                let group = dedupe_groups.entry(dup_group_id.clone()).or_insert_with(|| {
    ...
    fn clean_item(&self, raw: &RawSourceItem) -> CleanItem {
        ...
        if let Some(original_url) = raw.metadata.get("original_url").and_then(|value| value.as_str()) {
            let normalized_original_url = normalize_url(original_url);
            if !normalized_original_url.is_empty() && !reference_urls.contains(&normalized_original_url) {
    ...
    fn resolve_candidates(&self, clean: &CleanItem) -> ResolutionOutcome {
        ...
        if let Some(game_id) = self.assets.official_account_index.get(&clean.normalized_author) {
            add_match(game_id, &clean.normalized_author, "strong", "official_account", 0.99);
        }

        for (seed, game_id) in &self.assets.official_url_seed_index {
            if !clean.normalized_url.is_empty() && clean.normalized_url.to_lowercase().contains(seed) {
    ...
    fn attribute_event(&self, clean: &CleanItem) -> EventOutcome {
        ...
        EventOutcome {
            trace: TraceEntry {
                ...
                decision: event_type
                    .clone()
                    .unwrap_or_else(|| "no_event".to_string()),
                detail: json!({
